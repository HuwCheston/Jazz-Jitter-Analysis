...
class LinePlotLaggedLatency(vutils.BasePlot):
    ...
    def _create_plot(
        ...
        """
        return sns.relplot(
            data=self.df, col="trial", row='jitter', markers='o', hue="instrument", hue_order=['Keys', 'Drums'],
    ...
    def _format_ax(
        ...
        self.g.set(
            xticks=[num for num in range(0, 8)], xticklabels=[num for num in range(1, 9)],
            ylabel='', xlabel='', title='', ylim=(-0.3, 0.3)
        )
